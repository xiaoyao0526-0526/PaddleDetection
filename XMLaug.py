...
if __name__ == "__main__":
    ...
    AUG_XML_DIR = "./AUG_XML"  # 存储增强后的XML文件夹路径
    mkdir(AUG_XML_DIR)

    AUG_IMG_DIR = "./AUG_IMG"  # 存储增强后的影像文件夹路径
    mkdir(AUG_IMG_DIR)

    TRAIN_TXT_PATH = "./train.txt"  # 指定train.txt文件的路径

    AUGLOOP = 50 # 每张影像增强的数量
    ...
    for root, sub_folders, files in os.walk(XML_DIR):
        ...
        for name in files:
            ...
            for epoch in range(AUGLOOP):
                ...
                # 更新train.txt
                image_path = os.path.join(AUG_IMG_DIR, str(name[:-4]) + "_aug_" + str(epoch) + '.jpg')
                label_path = os.path.join(AUG_XML_DIR, str(name[:-4]) + "_aug_" + str(epoch) + '.xml')
                train_entry = f"{image_path} {label_path}\n"
                with open(TRAIN_TXT_PATH, "a") as train_file:
                    train_file.write(train_entry)
                new_bndbox_list = []
